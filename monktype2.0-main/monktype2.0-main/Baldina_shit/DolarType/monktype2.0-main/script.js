...
function updateGameTime(value) {
    const timeElement = document.querySelector('.option-element div.mode.time');
    const quotesElement = document.querySelector('.option-element div.mode.quotes');
    const numbersElement = document.querySelector('.option-element div.mode.numbers');
    const punctuationElement = document.querySelector('.option-element div.mode.punctuation');

    console.log('this shit works');
    console.log(timeElement);
    let currentGameMode = '';

    // Check for combinations of classes
    if (punctuationElement.classList.contains('white') && numbersElement.classList.contains('white') && timeElement.classList.contains('white')) {
        currentGameMode = 'combined';

        console.log('really combined');
    } else if (punctuationElement.classList.contains('white') && !numbersElement.classList.contains('white') && timeElement.classList.contains('white')) {
        currentGameMode = 'punctuation';

    } else if (numbersElement.classList.contains('white') && !punctuationElement.classList.contains('white') && timeElement.classList.contains('white')) {
        currentGameMode = 'numbers';
        
    }

   
        if (value === undefined || [15, 30, 60].includes(value)) {
            gameTime = (value || 15) * 1000;
            console.log('After gameTime calculation:', gameTime); // Log the updated value of gameTime
...
function formatQuote(quote) {
    ...
    const formattedWords = words.map(word => formatWord(word));
...
function resetGame() {
    // Reset other game-related properties as needed
    ...
    // Reset marginTop value to 274 pixels
...
function resetMarginTop() {
    ...
    if (words.getBoundingClientRect().top < 240) {
        const words = document.getElementById('words');
        const margin = parseInt(words.style.marginTop || '0px');
        console.log('this should activate also');
...
function newGame(option) {
    ...
    if (option === "numbers") {
        ...
        const formattedQuote = formatQuote(selectedQuote);

        document.getElementById('words').innerHTML = '';
        document.getElementById('words').innerHTML = formattedQuote;
...
function toggleColor(element) {
    const timeElement = document.querySelector('.option-element div.mode.time.white');
    const quotesElement = document.querySelector('.option-element div.mode.quotes.white');
    
    if (!element.classList.contains('white') && element.classList.contains('numbers') && timeElement.classList.contains('white'))  {    
        currentGameMode = 'numbers';
        newGame(currentGameMode);
        addClass(element, 'white');
        console.log("1st if");
    } else if (!element.classList.contains('white') && element.classList.contains('punctuation') && timeElement.classList.contains('white')) {
        currentGameMode = 'punctuation';
        newGame(currentGameMode);
        addClass(element, 'white');
        console.log("2nd if");
        ...
        addClass(element, 'white');
        newGame();
        removeClass(quotesElement, 'white');
    
    
    }else if(element.classList.contains('quotes') && !element.classList.contains('white')){
        currentGameMode = 'quotes';
        newGame(currentGameMode);
        addClass(element, 'white');
        console.log("4th if");
        const numbersElement = document.querySelector('.option-element div.mode.numbers.white');
        const punctuationElement = document.querySelector('.option-element div.mode.punctuation.white');
        const timeElement = document.querySelector('.option-element div.mode.time.white');
        removeClass(timeElement, 'white');
        removeClass(numbersElement, 'white');
        removeClass(punctuationElement, 'white');
    ...
    //problema ar sho varbuut jadzeesh buus
    else {
        currentGameMode = '';
    ...
    if (numbersElement && punctuationElement && timeElement) {
        currentGameMode = 'combined';
        newGame(currentGameMode);
        
        console.log("combined mode");
    }
    else if(numbersElement && timeElement){
        currentGameMode = 'numbers';
        newGame(currentGameMode);
        console.log("numbers mode");
    }
    else if(punctuationElement && timeElement){
        currentGameMode = 'punctuation';
        newGame(currentGameMode);
        console.log("punctuation mode");
    }
    else if(numbersElement && punctuationElement && timeElement){
        currentGameMode = 'combined';
        newGame(currentGameMode);
        console.log("combined mode");
    }
...
const selectedElement = document.querySelector('div.option-element div.mode.time.white');
document.getElementById('game').addEventListener('keydown', ev => {
    ...
    if (!window.timer && isLetter && selectedElement.classList.contains('time')) {
      ...
      if (isLetter && !currentLetter && !selectedElement.classList.contains('white')) {
        window.timer = setInterval(() => {
            if (!window.gameStart) {
              window.gameStart = (new Date()).getTime();
            }
            ...
            if (currentWord.nextSibling === null) {// buggy buggy
                console.log("quotes work");
              gameOver();
              return;
            }
    ...
    if (isSpace) {
        ...
        const isCurrentWordVisible = currentWord.previousSibling? currentWord.previousSibling.getBoundingClientRect().top > 270: true; 
        
        
        //console.log(isCurrentWordVisible + ' ' + currentWord.getBoundingClientRect().top );

        /*const currentWordTop = currentWord.previousSibling.lastChild.getBoundingClientRect().top;

        if (currentWordTop < 240) {
            // If the top margin is less than 240, do not execute the backspace logic
            return;
        }*/
